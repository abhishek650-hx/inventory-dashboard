...
def generate_timeseries(product_name, base_demand):
    ...
    seasonality += np.random.normal(0, 12, periods)

    # 🔥 extra irregular spikes
    spikes = np.random.choice([0, 1], size=periods, p=[0.9, 0.1])
    spike_values = spikes * np.random.uniform(-50, 50, periods)

    noise = np.random.normal(0, base_demand * 0.1, periods)

    y = base_demand + trend + seasonality + noise + spike_values
...
if menu == "Dashboard":
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            hover_data=["category", "available_quantity", "ROP"]
        )
    ...
    with col2:
        ...
        fig2 = px.bar(
            ...
            orientation='h',
            barmode='group',
            labels={"value": "Stock Level (Units)", "variable": "Metric"}
        )
    ...
    if forecast is not None:
        fig_forecast = px.line(
            ...
            x='ds',
            y='yhat',
            labels={"yhat": "Forecasted Demand (Units)"}
        )
